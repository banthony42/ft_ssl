...
/*   Updated: 2019/10/18 16:32:24 by banthony         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_ssl.h"

static int		ssl_store_option(char **values, t_cmd_opt *opt,
								size_t shift, int i)
{
								...
								t_cmd_opt *opt, t_parsing_param param)
{
	...
	if ((parsing_status = ssl_find_key_value(entry, values,
			param.opts_arg[index], opt)) != PARSING_CONTINUE)
	...
	while (values[++i])
		if (!(ft_strncmp(entry, values[i], entry_len)))
			if (entry_len == ft_strlen(values[i]))
				return (ssl_store_option(values, opt, shift, i));
	ft_freetab(values);
